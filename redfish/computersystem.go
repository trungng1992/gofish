...
type ComputerSystem struct {
	...
	storage string
	// smartStorage shall be a link to a collection
	// of type SmartStorageCollection
	smartStorage string
...
func (computersystem *ComputerSystem) UnmarshalJSON(b []byte) error {
	...
	*computersystem = ComputerSystem(t.temp)
	...
	if string(t.Oem.Hp.Links.SmartStorage) != "" {
		computersystem.smartStorage = string(t.Oem.Hp.Links.SmartStorage)
	}
	if string(t.Oem.Hpe.Links.SmartStorage) != "" {
		computersystem.smartStorage = string(t.Oem.Hpe.Links.SmartStorage)
	}

	if string(t.Oem.Hp.Links.Memory) != "" {
		computersystem.memory = string(t.Oem.Hp.Links.Memory)
	}

	if string(t.Oem.Hpe.Links.Memory) != "" {
		computersystem.memory = string(t.Oem.Hpe.Links.Memory)
	}

	if string(t.Links.Processors) != "" {
		computersystem.processors = string(t.Links.Processors)
	}

	if computersystem.smartStorage == "" {
